...
public class JavaFxGameTablero {
    ...
    public JavaFxGameTablero() {
                    ...
                    setCasilla();
                    Cliente_Servidor();
                    if(Servidorcliente == true) {
                       servidor = new Servidor(tablero,"");
                      }
                    else
                    { cliente = new Cliente(tablero);
                    }
...
*/
            ...
            /* ActualizarTablaFichas(servidor.getCasilla());
             ...
             servidor.setCadena("");*/
        } else if (servidorCliente && getTurno() == "negro" || clienteServidor && getTurno() == "blanco") {
    ...
    public boolean Movimiento(Casilla acasilla) {
        if (acasilla.isSombreada()) {
            ...
            if (turno == "blanco" && servidorCliente == true) {
                ...
                servidor = new Servidor(tablero,fichaMarcadaY + " " + fichaMarcadaX + " " + acasilla.getPosicionArregloX() / 100 + " "
                        + acasilla.getPosicionArregloY() / 100);
                turno = "negro";
               // servidor.start();
            } else if (turno == "negro" && clienteServidor == true) {
                //cliente.start();
                //cliente = new Cliente(tablero,fichaMarcadaY + " " + fichaMarcadaX + " " + acasilla.getPosicionArregloX() / 100 + " "
                       // + acasilla.getPosicionArregloY() / 100);
                turno = "blanco";
                //cliente.setCadena("");
   ...
   /* public Casilla DescomponerString(String dato)
    ...
    public String getCadenaEnviar() {
        return cadenaEnviar;
    }

    public void setCadenaEnviar(String cadenaEnviar) {
        this.cadenaEnviar = cadenaEnviar;
    }

    public boolean isServidorCliente() {
        return servidorCliente;
    }

    public void setServidorCliente(boolean aServidorcliente) {
        if (aServidorcliente) {
            servidorCliente = true;
